...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [
            [
                InlineKeyboardButton('🤖 UPDATE 🤖', url="https://telegram.dog/ANUSHKA_SEN_CHANNEL")
            ],
            [
                InlineKeyboardButton('♻️  ᴘʟᴇᴀꜱᴇ ꜱʜᴀʀᴇ  ♻️', url=f"https://telegram.dog/share/url?url=https://telegram.dog/ANUSHKA_SEN_CHANNEL"),
            ]
            ]
        ...
        if not await db.get_chat(message.chat.id):
            ...
            await client.send_message(LOG_CHANNEL, script.LOG_TEXT_G.format(message.chat.title, message.chat.id, message.chat.username, total, "Unknown", temp.B_MEN))       
            await db.add_chat(message.chat.id, message.chat.title)
        return 
    if not await db.is_user_exist(message.from_user.id):
        await db.add_user(message.from_user.id, message.from_user.first_name)
        await client.send_message(LOG_CHANNEL, script.LOG_TEXT_P.format(message.from_user.id, message.from_user.mention, message.from_user.username, temp.B_MEN))
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton('⇄  ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘ  ⇄', url=f'http://telegram.me/{temp.U_NAME}?startgroup=true')
            ],[
            InlineKeyboardButton('👨‍💻  ᴏᴡɴᴇʀ​', callback_data='admin'),
        ...
        await message.reply_photo(
            ...
            caption=script.START_TXT.format(message.from_user.mention, temp.B_MEN, temp.U_NAME, temp.B_NAME),
...
async def tutorial(bot, message):
    ...
    await reply.edit_text(f"𝙏𝙪𝙩𝙤𝙧𝙞𝙖𝙡 𝙎𝙪𝙘𝙘𝙚𝙨𝙨𝙛𝙪𝙡 𝘼𝙙𝙙𝙚𝙙\n\n<b>➥  ʏᴏᴜʀ ᴛᴜᴛᴏʀɪᴀʟ ʟɪɴᴋ ꜰᴏʀ {title} ɪs \n\n☞  <code>{tutorial}</code>\n\n★  ʙʏ :  <a href=https://telegram.dog/ANUSHKA_SEN_CHANNEL>ANUSHKA_SEN_CHANNEL</a></b>", disable_web_page_preview=True)
